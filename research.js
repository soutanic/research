...
function composite() {
    ...
    if (localStorage.getItem('key')!=null){
        var login_code = localStorage.getItem('key');
    }else{
        new_login = true;
        login_code = document.getElementById('login_code').value;
        var one_more = document.getElementById('code_none');
        if (one_more.checked==true) {
            localStorage.setItem('key', login_code);        
        }
    }
    if (login_code == ''){
        alert('ログインコードを入力してください');
    }else{
